...
pub fn make_bindings(
    ...
    for (index, function) in summary.functions.iter().enumerate() {
        ...
        let mut bindgen = FunctionBindgen::new(summary, function, stack_pointer);

        match function.kind {
            FunctionKind::Import => {
                bindgen.compile_import(import_index.try_into().unwrap());
                import_index += 1;
            }
            FunctionKind::Export => bindgen.compile_export(
                export_set
            ...
            FunctionKind::ExportFromCanon => bindgen.compile_export_from_canon(),
            FunctionKind::ExportToCanon => bindgen.compile_export_to_canon(),
            FunctionKind::ExportPostReturn => bindgen.compile_export_post_return(),
            FunctionKind::ResourceNew => {
                bindgen.compile_resource_new(import_index.try_into().unwrap());
                import_index += 1;
            }
            FunctionKind::ResourceRep => {
                bindgen.compile_resource_rep(import_index.try_into().unwrap());
                import_index += 1;
            }
            FunctionKind::ResourceDropLocal | FunctionKind::ResourceDropRemote => {
                bindgen.compile_resource_drop(import_index.try_into().unwrap());
                import_index += 1;
            }
        };

        let mut func = Function::new_with_locals_types(bindgen.local_types);
        for instruction in &bindgen.instructions {
