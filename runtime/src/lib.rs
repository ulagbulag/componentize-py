...
pub extern "C" fn componentize_py_from_canon_handle<'a>(
    ...
    if local != 0 {
        if borrow != 0 {
            ...
            let value = unsafe { Py::from_borrowed_ptr(*py, rep as _) }.into_bound(*py);
...
pub extern "C" fn componentize_py_to_canon_handle(
    ...
    if local != 0 {
        ...
        if value.hasattr(name).unwrap() {
            ...
            let rep = Py::from(value.to_owned()).into_ptr();
            let handle = {
            ...
            let instance: Py<PyAny> = unsafe { Py::from_borrowed_ptr(*py, rep) };
